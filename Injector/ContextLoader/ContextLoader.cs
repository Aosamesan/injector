...
namespace Injector.ContextLoader
{
    public class ContextLoader
    {
        ...
        public static IContext LoadContext(Type contextType)
        {
            ...
            var paramLessMethodInfos = contextType.GetMethods(BindingFlags.Public | BindingFlags.Instance).GetMethodBaseWithoutParameter<MethodInfo, Instantiate>();
            foreach (var methodInfo in paramLessMethodInfos)
            {
                Console.WriteLine("Parameterless Method : {0}", methodInfo.Name);
                var result = methodInfo.Invoke(contextInstance, null);
                var name = NamingHelper.ConvertToCamelCase(methodInfo.Name);
                Console.WriteLine("normalized name : {0}", name);
            ...
            var modulesToScan = currentAssembly.GetModules().Find(module => !string.IsNullOrEmpty(module.Name) && namespaceToScan.Contains(module.Name)); 
            ...
            var parameterMethodInfo = contextType.GetMethods(BindingFlags.Public | BindingFlags.Instance).GetMethodBaseWithParameter<MethodInfo, Instantiate>();
            foreach (var methodInfo in parameterMethodInfo)
            {
